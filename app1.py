    ...
    if st.button("解析を実行して評価する", key="analyze_btn", type="primary"):
        ...
        try:
            ...
            def make_safe(name):
                ...
                if re.match(r'^[0-9]', safe): safe = '_' + safe 
                return safe
            ...
            if "ノンパラメトリック" in dist:
                ...
                st.info("発病指数や食味評価のような「順序データ」は、数値の間隔に連続的な意味がありません。無理に正規分布とみなして平均値を比較すると実態と異なる結論を導く危険があります。データを「順位」に変換して比較するため安全な選択です。")
                
                # 👇 ノンパラの弱点とポアソンへの誘導を追記 👇
                st.markdown("#### 💡 さらなるステップアップ（別解の提案）")
                st.warning("""
                ノンパラメトリック検定は安全ですが、弱点もあります。それは**「どの要因がどのくらいスコアを上げたのか（影響の大きさ）」**や、**「複数の変数の影響（例：温度×肥料などの交互作用）」**を同時に評価できない点です。
                もしそうした複雑な解析を行いたい場合、このスコアデータを『一種のカウントデータ（0以上の整数）』とみなして、**ポアソン分布 (GLM)** を適用する高度なアプローチもあります。
                ぜひ、このデータで「ポアソン分布」を選び直して再実行してみてください！
                """)
                ...
                # 👇 スコアデータ（最大値が10以下の整数）にポアソンを選んだ場合の「特別フィードバック」を追加 👇
                if df[col_event].max() <= 10:
                    st.success("🎉 【高度な正解（別解）】順序データ（スコア）に対して、ポアソン分布（GLM）を適用する高度なアプローチです！")
                    st.markdown("#### 📖 なぜノンパラのデータ（順序データ）にポアソン回帰が使えるのか？")
                    st.info("""
                    ノンパラメトリック検定は「差の有無（p値）」しかわからず、「複数の変数の影響（交互作用など）」や「影響の大きさ」を計算できないという弱点があります。
                    
                    そこで、0以上の整数をとるスコアデータを**『一種のカウントデータ』とみなしてポアソン回帰（GLM）を適用する**ことで、要因ごとの影響力を数値化（モデル化）できるようになります。
                    
                    ※厳密にはスコアには上限（例: 5段階評価）があるため「順序ロジスティック回帰」などが最も適切ですが、実務や農業研究において、複数の要因をGLMでモデリングするための『実用的な近似手法』としてポアソン分布がよく使われます。
                    """)
                    st.write(model.summary())
                    st.markdown("#### 🔍 解析結果の読み方")
                    st.markdown("* **`coef`**: 要因がスコアをどれくらい変動させるか（自然対数の底 $e$ の累乗として計算）の指標になります。\n* **`P>|z|`**: 0.05未満であれば統計的に意味のある要因です。")
